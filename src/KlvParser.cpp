...
KLV* KlvParser::parseByte(uint8_t byte) {
    ...
    case STATE_INIT: {       // init state
        ...
        case KEY_ENCODING_1_BYTE: {
            state = STATE_KEY;
            DEBUG_PRINT("KlvParser transitioning to STATE_KEY\n");
            break;
        }
    ...
    case STATE_KEY: {       // read KLV 16-byte universal key
        ...
        if(ber_long_form) {
            ...
            val_len = 0;
            DEBUG_PRINT("BER-Len field is long-form\n");
            DEBUG_PRINT("BER len: %ld\n", ber_len);
            DEBUG_PRINT("KlvParser transitioning to STATE_LEN_HEADER\n");
        } else {
            state = STATE_LEN;
            val_len = byte & 0b01111111;
            DEBUG_PRINT("BER-Len field is short-form\n");
            DEBUG_PRINT("Value length: %ld\n", val_len);
            DEBUG_PRINT("KlvParser transitioning to STATE_LEN\n");
        }
        break;
    }
    ...
    case STATE_LEN: {       // read entire BER-encoded length field
        ...
        if(val.size() == val_len) {
            state = STATE_VALUE;
            DEBUG_PRINT("KlvParser transitioning to STATE_VALUE\n");
        } else {
            break;
        }
    }
    ...
    case STATE_VALUE: {      // read value field
        ...
        DEBUG_PRINT("key_encodings.size() : %ld\n", key_encodings.size());
        // key_encodings.erase(key_encodings.begin());
        if(key_encodings.size() > 1) {
            DEBUG_PRINT("Creating sub_klv_parser...\n");
